...
def get_all_restaurants():
    ...
    restaurants = Restaurant.query\
        .options(joinedload(Restaurant.menu_items)).filter(Restaurant.menu_items.any()).all()
...
def create_restaurant():
    ...
    if form.validate_on_submit():
        try:
            # Convert camelCase request data to snake_case
            restaurant_data = convert_camel_to_snake(form.to_dict())
...
def update_restaurant(restaurant_id):
    ...
    if restaurant.owner_id != current_user.id:  # ✅ Use snake_case
    ...
    if form.validate_on_submit():
        try:
            # Convert request data to snake_case before updating
            restaurant_data = convert_camel_to_snake(form.to_dict())
